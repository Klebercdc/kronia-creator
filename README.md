...
src/
  ...
  lib/
    openai.ts               — client OpenAI (texto + visão), provider único do pipeline — saída estruturada validada por Zod, retry em falha de schema
  core/
    ingestion/               — download (yt-dlp) → frames (ffmpeg) → transcript (legendas/Whisper) → visão (OpenAI)
    ...
    generation/               — roteirista, marketing, persuasão, cinematográfico, seo, teólogo, psicologia de compra (todos OpenAI)
      roteirista.ts · marketing.ts · teologo.ts · psicologia-compra.ts · persuasao.ts · cinematografico.ts · generate.ts (orquestrador)
    compliance/
      validate.ts (gate) · copyright-check.ts · correct.ts (correção direcionada às violações)
...
chamadas reais: Ingestão (yt-dlp + ffmpeg + Whisper + visão, tudo OpenAI) →
...
Provider único: OpenAI. Groq foi removido (usado até então pro grosso do
pipeline por ser grátis) depois de um incidente real em testes — a cota
diária gratuita se esgotou em uso normal, e o fallback pra OpenAI que
deveria cobrir esse caso tinha um bug de schema próprio. Manter dois
providers só valeria a pena se o custo de IA fosse o gargalo real do
produto; não é (ver `RELATORIO-AUDITORIA.md`) — confiabilidade > economia
teórica de centavos nesta fase. `npm run typecheck` passa limpo e o fluxo
