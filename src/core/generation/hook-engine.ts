import type { ClassificationResult, ContentRequest } from "../../types/pipeline";
...
const REFERENCE_FAMILIES:Record<ClassificationResult["hookType"],readonly HookFamily[]> = {
  curiosity:["curiosity","discovery","reveal"],
  pattern_interrupt:["pattern_interrupt","contrarian"],
  bold_statement:["contrarian","confession"],
  question:["curiosity","objection"],
  social_proof:["proof","confession"],
  before_after:["before_after","comparison"],
  negative_hook:["problem","objection"],
  relatable_pain:["problem","identity","pov"],
  story_open:["story","emotional"],
  result_first:["result","demo","reveal"],
  identity_call:["identity","pov"],
  number_stat:["number","list"],
};
...
function score(p:HookPattern,r:ContentRequest,classification:ClassificationResult|null){
  let s=0;
  ...
  if(classification && REFERENCE_FAMILIES[classification.hookType].includes(p.family)) s+=20;
  return s;
}

function selectionSeed(request:ContentRequest,classification:ClassificationResult|null){
  return JSON.stringify({
    project:request.project,
    objective:request.objective,
    mode:request.mode,
    productInfo:request.productInfo,
    hookType:classification?.hookType??null,
    persuasionMechanisms:classification?.persuasionMechanisms??[],
  });
}

/** Seleção determinística e contextual. A mesma entrada produz o mesmo
 * resultado, mas produtos e referências diferentes percorrem variantes
 * diferentes dentro de cada família. */
export function selectHookPatterns(
  ...
  const seed=selectionSeed(request,classification);
  const ranked=[...HOOK_LIBRARY].sort((a,b)=>{
    const scoreDiff=score(b,request,classification)-score(a,request,classification);
    if(scoreDiff!==0) return scoreDiff;
    const variantDiff=stableHash(`${seed}:${b.id}`)-stableHash(`${seed}:${a.id}`);
    return variantDiff||a.id-b.id;
  });

  const out:HookPattern[]=[];
  const families=new Set<HookFamily>();
  const limit=Math.min(count,FAMILIES.length);
...
export function buildHookIntelligenceBrief(
  request:ContentRequest,
  classification:ClassificationResult|null=null,
){
  const selected=selectHookPatterns(request,classification);
  const referenceDirective=classification
    ? `REFERÊNCIA ATIVA: preserve a mecânica ${classification.hookType}; as opções compatíveis receberam prioridade. Pacing: ${classification.pacing}. Mecanismos observados: ${classification.persuasionMechanisms.join(", ")||"nenhum"}.`
    : "SEM REFERÊNCIA: selecione pela combinação de objetivo, modo, projeto e evidências do pedido.";
  ...
  return [
    ...
    "",
    "Crie exatamente uma opção de hook para cada mecânica acima, mantendo a ordem.",
    ...
    "selectedHook deve ser exatamente uma das cinco opções geradas.",
